...
const LAYERS = [
  ...
  { src: cloudFront, factor: 0.52, scale: 1.3, opacity: 1 },
]

const HeroLayer = ({ layer, style, index }) => (
  <img
    src={layer.src}
    alt=""
...
export default function Hero() {
  ...
  const progress = Math.min(Math.max((scrollY - top) / range, 0), 1)

  const layerStyles = useMemo(() => 
    LAYERS.map(layer => ({
      transform: `translate3d(0, ${-progress * 100 * layer.factor}px, 0) scale(${layer.scale})`,
    ...
    <section id="topo" className="hero" ref={sectionRef}>
      <div className="hero__sticky">
        <div className="hero__layers" aria-hidden="true">
          {LAYERS.map((layer, i) => (
            <HeroLayer key={i} layer={layer} style={layerStyles[i]} index={i} />
          ))}
